"""
Module: get_vacancy
Description: Retrieves a single vacancy using gRPC.
Author: oaslananka
...
class VacancyServiceClient:
    ...
    def get_vacancy(self, vacancy_id):
        ...
        Args:
            vacancy_id (str): The ID of the vacancy to retrieve.
        ...
        try:
            ...
            logging.info(f"GetVacancy successful: {response}")
            return response
...
def main():
    ...
    for vacancy in vacancies:
        ...
        if vacancy_id:
            response = vacancy_service_client.get_vacancy(vacancy_id)
            if response:
                logging.info(f"Retrieved vacancy: {response}")
            else:
                logging.error(f"Failed to retrieve vacancy ID: {vacancy_id}")
